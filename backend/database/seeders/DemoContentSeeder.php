...
class DemoContentSeeder extends Seeder
{
    public function run(): void
    {
        $admin = User::query()->where('username', 'admin')->first()
            ?? User::query()->where('email', 'admin@taghvim.local')->first();

        if (! $admin) {
            $this->command?->warn('DemoContentSeeder: admin user not found; skipped.');

            return;
        }

        $editor = User::query()->where('username', 'editor')->first() ?? $admin;
        ...
        $this->command?->info("DemoContentSeeder: seeded {$createdDays} days and {$createdEvents} events.");
    }

    private function purgeDayActions(CalendarDay $day): void
    {
        $day->enemyActions()->withTrashed()->get()->each(function (EnemyAction $action): void {
            $this->purgeModelMedia($action);
            $action->forceDelete();
        });

        $day->governmentActions()->withTrashed()->get()->each(function (GovernmentAction $action): void {
            $this->purgeModelMedia($action);
            $action->forceDelete();
        });
    }

    private function purgeModelMedia(Model $model): void
    {
     ...
     *   title: string,
     *   summary: string,
     *   enemy: list<array{title: string, severity: ActionSeverity, category: string, location?: string}>,
     *   government: list<array{title: string, agency: string, category: string}>
     * }>
     */
    private function dayDefinitions(): array
    {
        return [
            [
                'offset' => 0,
